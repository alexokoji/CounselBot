...
def fetch_user_data():
    ...
    print("Fetched credentials:", credentials)  # Debug print
    return credentials

credentials = fetch_user_data()
...
config = {
    ...
    'preauthorized': {
        'emails': [credentials['usernames'][user]['email'] for user in credentials['usernames']]
    }
}

# Load the configuration into the authenticator
...
def register_user(username, email, name, password, security_question, security_answer):
    hashed_password = password
    cursor.execute('INSERT INTO users (username, email, name, password, security_question, security_answer) VALUES (?, ?, ?, ?, ?, ?)',
                   (username, email, name, hashed_password, security_question, security_answer))
    conn.commit()
...
def main():
    st.title("Career Counseling System")
    ...
    if choice == "Login":
        ...
        if authentication_status:
            ...
            st.write("Hi! I'm Career Counseling bot, your personal career counseling bot. Ask your queries in the text box below and hit enter. If and when you are ready to take our personality test, type \"start my test\" and you're good to go!")

            df3 = get_text()
            if (df3.loc[0, 'user']==""):
                ans = "Hi, I'm Career Counseling bot. \nHow can I help you?"
            ...
            st.text_area("Career Counseling System:", value=ans, height=100, max_chars=None)
        ...
        if password == password2:
            if st.button("Register"):
                register_user(username, email, name, password, security_question, security_answer)
        else:
                                                ...
                                                if (inp8 != "Select an Option"):
                                                    ...
                                                    if (inp9 != "Select an Option"):
                                                        ...
                                                        if (inp10 != "Select an Option"):
                                                            ...
                                                            for i in range(0, 5):
                                                                st.subheader(professions[int(l[i])])
                                                                st.write("NGN. " + str(df['Income'][int(l[i]) - 1]))
